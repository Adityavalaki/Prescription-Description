...
export function Logo({ size = 28, mono = false }) {
  return (
    <View style={{ flexDirection: 'row', alignItems: 'center', gap: 9 }}>
      <View style={{ width: size * 1.2, height: size * 1.2, borderRadius: size * 1.2 * 0.38, backgroundColor: mono ? C.mint : C.primary, alignItems: 'center', justifyContent: 'center' }}>
        <Icon name="plus" size={size * 0.66} color={mono ? C.deep : '#fff'} stroke={3} />
      </View>
      <Text style={{ fontFamily: F.display, fontSize: size, letterSpacing: -0.6, color: mono ? '#fff' : C.deep }}>Medira</Text>
    </View>
  );
}
